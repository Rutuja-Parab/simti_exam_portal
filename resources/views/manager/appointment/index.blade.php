        ...
        <section class="content">
            ...
            <div class="row">
                <div class="col-12 col-lg-12">
                    <h4><a href="{{ route('manager.appointment.create') }}"
                            class="btn btn-success">{{ __('manager/menu.appointment_create') }}</a>
                    </h4>
                </div>
                    ...
                    <table id="data-table" class="table table-striped">
                        ...
                        <tbody>
                            @foreach ($appointments as $appointment)
                                <tr>
                                    <th scope="row">{{ $appointment->user->name . ' ' . $appointment->user->surname }}
                                    </th>
                                    <td>{{ $appointment->teacher->name . ' ' . $appointment->teacher->surname }}</td>
                                    <td>{{ $appointment->car->plate_code }}</td>
                                    <td>{{ $appointment->date }}</td>
                                    <td class="{{ $appointment->status == 1 ? 'text-success' : 'text-danger' }} fw-bold">
                                        {{ $appointment->status === 1 ? 'Active' : 'Inactive' }}</td>
                                    <td>
